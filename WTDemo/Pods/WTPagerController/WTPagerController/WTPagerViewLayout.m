...
typedef NS_ENUM(NSUInteger, WTPagerScrollingDirection) {
    WTPagerScrollingLeft,
    WTPagerScrollingRight,
};
...
@interface WTPagerViewLayout<ItemType> ()<UIScrollViewDelegate> {
    // Private
...
@implementation WTPagerViewLayout
...
- (void)setDataSource:(id<WTPagerViewLayoutDataSource>)dataSource {
...
- (void)setDelegate:(id<WTPagerViewLayoutDelegate>)delegate {
    _delegate = delegate;
...
- (void)caculateIndexWithOffsetX:(CGFloat)offsetX direction:(WTPagerScrollingDirection)direction{
    if (CGRectIsEmpty(_scrollView.frame)) {
        return;
    }
    if (_countOfPagerItems <= 0) {
    ...
    // caculate cur index
    if (direction == WTPagerScrollingLeft) {
...
- (void)caculateIndexByProgressWithOffsetX:(CGFloat)offsetX direction:(WTPagerScrollingDirection)direction{
    if (CGRectIsEmpty(_scrollView.frame)) {
        return;
    }
    if (_countOfPagerItems <= 0) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    WTPagerScrollingDirection direction = offsetX >= _preOffsetX ? WTPagerScrollingLeft : WTPagerScrollingRight;
