...
@interface WTTabPagerBarLayout ()

@property (nonatomic, weak) WTTabPagerBar *pagerTabBar;
...
@implementation WTTabPagerBarLayout

- (instancetype)initWithPagerTabBar:(WTTabPagerBar *)pagerTabBar {
    if (self = [super init]) {
        _pagerTabBar = pagerTabBar;
        [self configurePropertys];
        self.barStyle = WTPagerBarStyleProgressElasticView;
    }
    return self;
}
...
- (void)setProgressHeight:(CGFloat)progressHeight {
    ...
    frame.origin.y = _barStyle == WTPagerBarStyleCoverView ? (height - _progressHeight)/2:(height - _progressHeight - _progressVerEdging);
    frame.size.height = progressHeight;
    _pagerTabBar.progressView.frame = frame;
}

- (UIEdgeInsets)sectionInset {
    if (!UIEdgeInsetsEqualToEdgeInsets(_sectionInset, UIEdgeInsetsZero) || _barStyle != WTPagerBarStyleCoverView) {
        return _sectionInset;
    }
    if (_barStyle == WTPagerBarStyleCoverView && _adjustContentCellsCenter) {
        return _sectionInset;
    }
...
- (void)setBarStyle:(WTPagerBarStyle)barStyle
{
    if (barStyle == _barStyle) {
        return;
    }
    if (_barStyle == WTPagerBarStyleCoverView) {
        self.progressBorderWidth = 0;
        self.progressBorderColor = nil;
    }
    _barStyle = barStyle;
    switch (barStyle) {
        case WTPagerBarStyleProgressView:
            self.progressWidth = 0;
            self.progressHorEdging = 6;
            self.progressVerEdging = 0;
            self.progressHeight = kUnderLineViewHeight;
            break;
        case WTPagerBarStyleProgressBounceView:
        case WTPagerBarStyleProgressElasticView:
            self.progressWidth = 30;
            self.progressVerEdging = 0;
            self.progressHorEdging = 0;
            self.progressHeight = kUnderLineViewHeight;
            break;
        case WTPagerBarStyleCoverView:
            self.progressWidth = 0;
    ...
    _pagerTabBar.progressView.hidden = barStyle == WTPagerBarStyleNoneView;
    if (barStyle == WTPagerBarStyleCoverView) {
        _progressRadius = 0;
...
- (void)transitionFromCell:(UICollectionViewCell<WTTabPagerBarCellProtocol> *)fromCell toCell:(UICollectionViewCell<WTTabPagerBarCellProtocol> *)toCell animate:(BOOL)animate {
...
- (void)transitionFromCell:(UICollectionViewCell<WTTabPagerBarCellProtocol> *)fromCell toCell:(UICollectionViewCell<WTTabPagerBarCellProtocol> *)toCell progress:(CGFloat)progress {
...
- (void)setUnderLineFrameWithIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    CGFloat progressY = _barStyle == WTPagerBarStyleCoverView ? (cellFrame.size.height - _progressHeight)/2:(cellFrame.size.height - _progressHeight - _progressVerEdging);
...
- (void)setUnderLineFrameWithfromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex progress:(CGFloat)progress
{
    ...
    CGFloat progressY = _barStyle == WTPagerBarStyleCoverView ? (toCellFrame.size.height - _progressHeight)/2:(toCellFrame.size.height - _progressHeight - _progressVerEdging);
    CGFloat progressX = 0, width = 0;
    
    if (_barStyle == WTPagerBarStyleProgressBounceView) {
    ...
    }else if (_barStyle == WTPagerBarStyleProgressElasticView) {
        if (fromCellFrame.origin.x < toCellFrame.origin.x) {
            if (progress <= 0.5) {
