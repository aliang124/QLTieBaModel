...
- (void)setProgressView:(UIView *)progressView {
    ...
    if (_layout && _layout.barStyle == WTPagerBarStyleCoverView) {
...
- (void)setDelegate:(id<WTTabPagerBarDelegate>)delegate {
    _delegate = delegate;
...
- (void)setLayout:(WTTabPagerBarLayout *)layout {
...
- (WTTabPagerBarLayout *)layout {
    if (!_layout) {
        _layout = [[WTTabPagerBarLayout alloc]initWithPagerTabBar:self];
    }
    return _layout;
}
...
- (__kindof UICollectionViewCell<WTTabPagerBarCellProtocol> *)dequeueReusableCellWithReuseIdentifier:(NSString *)identifier forIndex:(NSInteger)index {
    UICollectionViewCell<WTTabPagerBarCellProtocol> *cell = [_collectionView dequeueReusableCellWithReuseIdentifier:identifier forIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
    return cell;
}
...
- (UICollectionViewCell<WTTabPagerBarCellProtocol> *)cellForIndex:(NSInteger)index
{
    if (index >= _countOfItems) {
        return nil;
    }
    return (UICollectionViewCell<WTTabPagerBarCellProtocol> *)[_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
}
...
{
    UICollectionViewCell<WTTabPagerBarCellProtocol> *cell = [_dataSource pagerTabBar:self cellForItemAtIndex:indexPath.item];
...
{
    UICollectionViewCell<WTTabPagerBarCellProtocol> *fromCell = [self cellForIndex:fromIndex];
    UICollectionViewCell<WTTabPagerBarCellProtocol> *toCell = [self cellForIndex:toIndex];
