...
+ (UIImage *)getJPEGImagerImg:(UIImage *)image {
    ...
    UIImage *newImg = [WTImageUtil imageWithImage:image scaledToSize:CGSizeMake(oldImg_WID, oldImg_HEI)];
    NSData *dJpeg = nil;
...
+ (NSArray *)getJPEGImagerImgArr:(NSArray *)imageArr {
    ...
    for (int i = 0; i<imageArr.count; i++) {
        UIImage *newImg = [WTImageUtil getJPEGImagerImg:imageArr[i]];
...
+ (NSArray *)getJPEGImagerImgArr:(NSArray *)imageArr compressibilityFactor:(CGFloat)compressibilityFactor {
    NSMutableArray *newImgArr = [NSMutableArray new];
    for (int i = 0; i<imageArr.count; i++) {
        UIImage *newImg = [WTImageUtil getJPEGImagerImg:imageArr[i] compressibilityFactor:compressibilityFactor];
        [newImgArr addObject:newImg];
    }
    return newImgArr;
}
