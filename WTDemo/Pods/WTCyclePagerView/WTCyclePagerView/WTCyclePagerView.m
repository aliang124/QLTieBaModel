...
NS_INLINE BOOL TYEqualIndexSection(WTIndexSection indexSection1,WTIndexSection indexSection2) {
    return indexSection1.index == indexSection2.index && indexSection1.section == indexSection2.section;
}

NS_INLINE WTIndexSection TYMakeIndexSection(NSInteger index, NSInteger section) {
    WTIndexSection indexSection;
    indexSection.index = index;
    indexSection.section = section;
    return indexSection;
}

@interface WTCyclePagerView () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, WTCyclePagerTransformLayoutDelegate> {
    struct {
...
@property (nonatomic, assign) WTIndexSection indexSection; // current index
@property (nonatomic, assign) NSInteger dequeueSection;
@property (nonatomic, assign) WTIndexSection beginDragIndexSection;
...
@implementation WTCyclePagerView
...
- (void)addCollectionView {
    WTCyclePagerTransformLayout *layout = [[WTCyclePagerTransformLayout alloc]init];
...
- (void)timerFired:(NSTimer *)timer {
    ...
    [self scrollToNearlyIndexAtDirection:WTPagerScrollDirectionRight animate:YES];
}

#pragma mark - getter

- (WTCyclePagerViewLayout *)layout {
    if (!_layout) {
...
- (void)setDelegate:(id<WTCyclePagerViewDelegate>)delegate {
    _delegate = delegate;
    ...
    if (self.collectionView && self.collectionView.collectionViewLayout) {
        ((WTCyclePagerTransformLayout *)self.collectionView.collectionViewLayout).delegate = _delegateFlags.applyTransformToAttributes ? self : nil;
    }
}

- (void)setDataSource:(id<WTCyclePagerViewDataSource>)dataSource {
...
- (void)scrollToNearlyIndexAtDirection:(WTPagerScrollDirection)direction animate:(BOOL)animate {
    WTIndexSection indexSection = [self nearlyIndexPathAtDirection:direction];
...
- (void)scrollToItemAtIndexSection:(WTIndexSection)indexSection animate:(BOOL)animate {
...
- (void)updateLayout {
    ...
    ((WTCyclePagerTransformLayout *)_collectionView.collectionViewLayout).layout = self.layout;
}
...
- (BOOL)isValidIndexSection:(WTIndexSection)indexSection {
...
- (WTIndexSection)nearlyIndexPathAtDirection:(WTPagerScrollDirection)direction{
    return [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
}

- (WTIndexSection)nearlyIndexPathForIndexSection:(WTIndexSection)indexSection direction:(WTPagerScrollDirection)direction {
    if (indexSection.index < 0 || indexSection.index >= _numberOfItems) {
        return indexSection;
    }
    
    if (!_isInfiniteLoop) {
        if (direction == WTPagerScrollDirectionRight && indexSection.index == _numberOfItems - 1) {
            return _autoScrollInterval > 0 ? TYMakeIndexSection(0, 0) : indexSection;
        } else if (direction == WTPagerScrollDirectionRight) {
    ...
    if (direction == WTPagerScrollDirectionRight) {
...
- (WTIndexSection)caculateIndexSectionWithOffsetX:(CGFloat)offsetX {
    if (_numberOfItems <= 0) {
...
- (CGFloat)caculateOffsetXAtIndexSection:(WTIndexSection)indexSection{
    if (_numberOfItems == 0) {
        return 0;
    }
    UICollectionViewFlowLayout *layout = (UICollectionViewFlowLayout *)_collectionView.collectionViewLayout;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    WTIndexSection newIndexSection =  [self caculateIndexSectionWithOffsetX:scrollView.contentOffset.x];
    ...
    WTIndexSection indexSection = _indexSection;
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    ...
    WTPagerScrollDirection direction = WTPagerScrollDirectionRight;
    if ((scrollView.contentOffset.x < 0 && targetContentOffset->x <= 0) || (targetContentOffset->x < scrollView.contentOffset.x && scrollView.contentOffset.x < scrollView.contentSize.width - scrollView.frame.size.width)) {
        direction = WTPagerScrollDirectionLeft;
    }
    WTIndexSection indexSection = [self nearlyIndexPathForIndexSection:_indexSection direction:direction];
...
#pragma mark - WTCyclePagerTransformLayoutDelegate
...
- (void)dealloc {
    ((WTCyclePagerTransformLayout *)_collectionView.collectionViewLayout).delegate = nil;
