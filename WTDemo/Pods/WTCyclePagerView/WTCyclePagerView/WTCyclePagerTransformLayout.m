...
typedef NS_ENUM(NSUInteger, WTTransformLayoutItemDirection) {
    WTTransformLayoutItemLeft,
    WTTransformLayoutItemCenter,
    WTTransformLayoutItemRight,
};


@interface WTCyclePagerTransformLayout () {
    struct {
...
@interface WTCyclePagerViewLayout ()
...
@implementation WTCyclePagerTransformLayout
...
- (void)setDelegate:(id<WTCyclePagerTransformLayoutDelegate>)delegate {
    _delegate = delegate;
...
- (void)setLayout:(WTCyclePagerViewLayout *)layout {
    _layout = layout;
...
- (WTTransformLayoutItemDirection)directionWithCenterX:(CGFloat)centerX {
    WTTransformLayoutItemDirection direction= WTTransformLayoutItemRight;
    ...
    if (ABS(centerX - contentCenterX) < 0.5) {
        direction = WTTransformLayoutItemCenter;
    }else if (centerX - contentCenterX < 0) {
        direction = WTTransformLayoutItemLeft;
    }
    return direction;
}
...
{
    return _layout.layoutType == WTCyclePagerTransformLayoutNormal ? [super shouldInvalidateLayoutForBoundsChange:newBounds] : YES;
}

- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    if (_delegateFlags.applyTransformToAttributes || _layout.layoutType != WTCyclePagerTransformLayoutNormal) {
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    }else if(_layout.layoutType != WTCyclePagerTransformLayoutNormal){
...
- (void)initializeTransformAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(WTCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case WTCyclePagerTransformLayoutLinear:
            [self applyLinearTransformToAttributes:attributes scale:_layout.minimumScale alpha:_layout.minimumAlpha];
            break;
        case WTCyclePagerTransformLayoutCoverflow:
        {
...
- (void)applyTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes layoutType:(WTCyclePagerTransformLayoutType)layoutType {
    switch (layoutType) {
        case WTCyclePagerTransformLayoutLinear:
...
- (void)applyLinearTransformToAttributes:(UICollectionViewLayoutAttributes *)attributes scale:(CGFloat)scale alpha:(CGFloat)alpha {
    ...
    if (_layout.adjustSpacingWhenScroling) {
        WTTransformLayoutItemDirection direction = [self directionWithCenterX:attributes.center.x];
        CGFloat translate = 0;
        switch (direction) {
            case WTTransformLayoutItemLeft:
                translate = 1.15 * attributes.size.width*(1-scale)/2;
                break;
            case WTTransformLayoutItemRight:
                translate = -1.15 * attributes.size.width*(1-scale)/2;
                break;
            default:
                // center
...
@implementation WTCyclePagerViewLayout
